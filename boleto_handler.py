...
def processar_resposta_boleto(resposta, contexto="boleto"):
    ...
    if contexto == "boleto":
        if resposta == "sim":
            return "Boleto enviado com sucesso! Posso ajudar com mais alguma coisa?"
    ...
    return "Desculpe, não entendi sua resposta. Por favor, tente novamente."
...
def processar_consulta_boletos():
    ...
    if numero_boletos == 1:
        mensagem = f"Você tem 1 boleto em aberto no valor de R$ {valor_total:.2f}.\n\n" \
                  "Gostaria de receber o boleto atualizado? (sim/não)"
        return mensagem, valor_total, "boleto"
    else:
